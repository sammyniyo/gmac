    ...
    <x-slot name="header">
        <div class="shadcn-page-head">
            <div>
                ...
                <h2 class="shadcn-title">{{ __('Add New Post') }}</h2>
                <p class="shadcn-desc">Write with the rich text editor below. Content is saved as HTML and shown on the public site.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="shadcn-card p-6">
                <form action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data" id="news-form">
                    ...
                    <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
                        <div class="lg:col-span-2 space-y-5">
                            <div>
                                ...
                                <input type="text" name="title" id="title" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm shadow-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20" value="{{ old('title') }}" required>
                                @error('title') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <label for="excerpt" class="mb-1.5 block text-sm font-medium text-gray-900">Excerpt</label>
                                <textarea name="excerpt" id="excerpt" rows="3" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm shadow-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20" placeholder="Short summary for listings…">{{ old('excerpt') }}</textarea>
                                @error('excerpt') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                            </div>

                            <div>
                                <span class="mb-1.5 block text-sm font-medium text-gray-900">Body <span class="text-red-500">*</span></span>
                                <input id="news_content" type="hidden" name="content" value="{{ e(old('content', '')) }}">
                        ...
                        <div class="space-y-5">
                            <div class="rounded-xl border border-gray-200 bg-gray-50/80 p-4">
                                ...
                                <label class="mb-4 flex items-center gap-2 text-sm">
                                    <input type="checkbox" name="is_published" value="1" class="rounded border-gray-300 text-teal-600 focus:ring-teal-500" {{ old('is_published', true) ? 'checked' : '' }}>
                                    <span class="font-medium">Published</span>
                                </label>
                                <div>
                                    <label for="published_at" class="mb-1 block text-xs font-medium text-gray-600">Publish date</label>
                                    <input type="date" name="published_at" id="published_at" class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" value="{{ old('published_at', now()->format('Y-m-d')) }}">
                            ...
                            <div class="rounded-xl border border-gray-200 bg-gray-50/80 p-4">
                                ...
                                <p class="mt-1 text-xs text-muted-foreground">JPG or PNG, max 2MB.</p>
                    ...
                    <div class="mt-8 flex flex-wrap items-center justify-end gap-3 border-t border-gray-200 pt-6">
                        ...
                        <button type="submit" class="shadcn-btn">Save post</button>
                    </div>
                </form>
            </div>
        </div>
